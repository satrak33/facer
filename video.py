...
VIDEO_SOURCE = Path('videos/short.mp4')  # Джерело відео
SCALE_FACTOR = 1  # Коефіцієнт зменшення зображення (1 - без змін)
FRAME_SKIP = 1  # Пропуск кадрів для обробки (щоб не перевантажувати процесор)
...
class FaceRecognition:
    ...
    def process_frame(self, frame):
        ...
        for (top, right, bottom, left), name in zip(face_locations, face_names):
            ...
            # cv2.imwrite(f"faces/{datetime.datetime.now().strftime('%H-%M-%S-%f')}.jpg", frame[top:bottom, left:right])
    ...
    def run_recognition(self):
        ...
        fps = int(vid.get(cv2.CAP_PROP_FPS)/FRAME_SKIP)
        output_file = "output.mp4"

        # Ініціалізація відеозаписувача
        fourcc = cv2.VideoWriter.fourcc(*'mp4v')  # Кодек (наприклад, XVID або MJPG)
        ...
        while True:
            ret, frame = vid.read()
            if not ret:
                log.success("Відео закінчилося :D")
                break
